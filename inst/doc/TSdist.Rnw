...
%\VignetteDepends{TSdist}
%\VignetteKeywords{timeseries, distance measures, R}
...
\title{Distance Measures for Time Series in \proglang{R}: The \pkg{TSdist} Package}  
...
These distances are implemented in the \code{manhattanDistance()}, \code{minkowskiDistance()}, \code{\allowbreak euclideanDistance()} and \code{infinitenormDistance()} functions. They receive two time series, represented by numeric vectors as the only input, except the Minkowski distance which also needs a specification of \code{p}. The output of these functions is the distance between the two series.


\begin{table}[h]
\begin{center}
\begin{tabular}{C{4cm}  C{4cm}  C{4cm}}  \hline
Distance       &  p & Formula   \\ \hline \\ [-1.5ex]
Manhattan    & $p=1$ &  $\sum_{i=0}^{N-1} {|x_i-y_i|}$  \\ [1ex] \\ [-1.5ex]
Minkowski     & $1<p<\inf$ &  $\sqrt[p]{\sum_{i=0}^{N-1} {(x_i-y_i)^{\frac{1}{p}}}}$  \\ [1ex]  \\ [-1.5ex]
Euclidean     & $p=2 $ &  $\sqrt{\sum_{i=0}^{N-1} {(x_i-y_i)^2}}$  \\ [1ex] \\ [-1.5ex]
Infinite norm    & $p=\inf$ &  $\operatorname*{max}_{i=0,...,N-1}{|x_i-y_i|}$  \\ [1ex]  \hline \\ [-1.5ex] 
\end{tabular}
\caption{$L_p$ distances.}
\label{Lp}
\end{center}
\end{table}
...
If the temporal indexes are not specified, a constant sampling rate will be assumed. 


\subsubsection{Dissim Distance}

The Dissim distance was introduced by \citep{Frentzos2007} and is specifically designed for series collected at different sampling rates. This means that each series will be defined in a finite set of time instants, but these can be different for each series.
...
The Dissim distance requires a continuous representation of the series and so, the series that are being compared are assumed to be linear between sampling points (see Figure~\ref{fig:dissim}). Once this is done, the definite integral of the Euclidean distance between them is calculated:
...
In order to overcome the inconveniences of rigid distances such as Euclidean Distance, many similarity measures have been specifically designed for time series data. Among them the most popular is probably Dynamic Time Warping (DTW) \citep{Berndt1994}. This distance is able to deal with transformations such as local warping and shifting and, furthermore, it allows the comparison between series of different length.

\begin{figure}[h]
\centering
\begin{tikzpicture}
\node[above right] (img) at (0,0) {\includegraphics[width=0.4\textwidth]{DTW2.pdf}};
\node at (230pt,130pt) {$d(x_i,y_j)$};
\node at (50pt,-1pt) {\footnotesize{$0$}};
\node at (178pt,-1pt) {\footnotesize{$N-1$}};
\node[rotate=90] at (41.5pt,44pt) {\footnotesize{$0$}};
\node[rotate=90] at (41.5pt,170pt) {\footnotesize{$M-1$}};
\node[text=blue] at (120pt,30pt) {\large{$\mathbf{X}$}};
\node[text=upforestgreen,rotate=90] at (7pt,115pt) {\large{$\mathbf{Y}$}};
\draw[thick,->] (7.3,4.5) -- (5.4,3.5);
\end{tikzpicture}
...
This optimization problem is subject to three restrictions \citep{WarrenLiao2005}. The boundary condition forces the path to start in position $D(0,0)$ and to end 
in $D(N-1,M-1)$. The continuity condition restricts the step size, forcing the path to continue through one of the adjacent cells. Finally, 
the monotonicity condition forbids the path to move backwards in the positions of the matrix. Based on this, the problem is reduced to solving the following recurrence:

{\small
\begin{equation}\label{eq:edr}
DTW(X,Y) = \left\{
\begin{array}{c l}
 0 & \text{if} \,\, M-1=N-1=0\\ [1ex]
\inf & \text{if} \,\, M-1=0 \,\, \text{or} \,\, N-1=0\\ [1ex]
...
{\noindent where $d$ is the Euclidean distance and, being $X=\{x_0,x_1,...,x_{N-1}\}$ and $Y=\{y_0,y_1,...,y_{M-1}\}$,  $Rest(X)$ and $Rest(Y)$ are defined as $\{x_1,...,x_{N-1}\}$ and $\{y_1,...,\allowbreak y_{M-1}\}$. By applying dynamic programming, this recurrence can be solved as shown in Algorithm~\ref{DTWal}.}
...
\centering
\begin{tikzpicture}
\node[above right] (img) at (0,0) {\includegraphics[width=0.4\textwidth]{DTW.pdf}};
\node at (230pt,130pt) {$d(x_i,y_j)$};
\node at (50pt,-1pt) {\footnotesize{$0$}};
\node at (178pt,-1pt) {\footnotesize{$N-1$}};
\node[rotate=90] at (41.5pt,44pt) {\footnotesize{$0$}};
...
Additionally, it must be noted that it is quite common to add an extra temporal constraint to DTW by limiting the number of vertical or horizontal steps that the path can take consecutively. The windowing type implemented in this package is the classical Sakoe-Chiba band \citep{sakoe_dynamic_1978} which places a symmetric band around the main diagonal and forces the path to stay inside this band (see Figure~\ref{fig:window}). This adjustment avoids the matching of points that are very far from each other in time and, in addition, it reduces the computation cost \citep{Wang2012}.
...
The second edit based distance included in the \pkg{TSdist} package, which is in fact a similarity measure, is the Longest Common Subsequence  distance (LCSS) \citep{Vlachos}. In this case, the similarity between two time series is quantified in terms of the longest common sub-sequence but, taking into account that  gaps or unmatched regions are permitted.
...
LCSS(X,Y) = \left\{
\begin{array}{c l}
 0 & \text{if} \,\, M-1=0 \, \, \text{or} \\
 & \, \, N-1=0 \\ [1ex]
 LCSS(Rest(X),Rest(Y))+1 & \text{if} \,\, |x_0-y_0| \leq \epsilon \\ [1ex]
...
The third adaptation to the edit distance is Edit Distance with Real Penalty (EDR) \citep{Chen2004} which is a combination of DTW and EDR. 
...
ERP(X,Y) = \left\{
\begin{array}{c l}
 \sum_{i=0}^{N-1} |y_i-g| & \text{if} \,\, M-1=0\\ [1ex]
 \sum_{i=0}^{M-1} |x_i-g| & \text{if} \,\, N-1=0\\ [1ex]
...
\begin{figure}[h]
       ...
       \quad \hspace{0.5cm}
        ...
        \begin{subfigure}[b]{0.3\textwidth}
                ...
                \caption{example.series2.}
                \label{cds}
        \end{subfigure}
        ~ %add desired spacing between images, e. g. ~, \quad, \qquad etc.
          %(or a blank line to force the subfigure onto a new line)
 
        \caption{The two example series of the same length included in the TSdist package.}\label{example12}
\end{figure}
...
\begin{figure}[h]
       ...
       \quad \hspace{0.5cm}
        \begin{subfigure}[b]{0.3\textwidth}
                ...
                \caption{example.series4.}
                \label{}
        \end{subfigure}
        ~ %add desired spacing between images, e. g. ~, \quad, \qquad etc.
          %(or a blank line to force the subfigure onto a new line)
 
        \caption{The two example series of different length included in the TSdist package.}\label{example34}
\end{figure}
...
The basic calculation of the distance between two series such as \code{example.series1} and \code{example.series2} is done as follows:
...
\begin{example}
A special case are the $L_p$ distances that can also be called by the wrapper function \code{lpDistance}:
...
Some distance measures can be computed between series of different lengths but in some other cases this is not possible. In these latter cases, the distance can not be computed and the function will return \code{NA} and an error message:
...
\begin{example}
Other errors will be specific to each distance measure and will be determined based on their definition. As in the previous case, when encountering an error, a \code{NA} value will be returned and the corresponding error message will be printed. As an example, the window function defined for DTW can not exceed the size of the time series being compared.
...
% \begin{Schunk}
% \begin{Sinput}
% R> dtwDistance(example.series3, example.series4, sigma=10)
% \end{Sinput}
% \begin{Soutput}
% [1] NA
% Error : The window size can not be lower than the difference between the series lengths
% 
% \end{Soutput}
% \end{Schunk}
\end{example}

\section[wrapper]{The use of time series objects of \proglang{R} in \pkg{TSdist}} \label{wrapper}

Given their relevance and special characteristics, many specific classes or object types have been included in \proglang{R} to define time series. The most popular examples are the \code{ts} objects introduced in the basic \pkg{stats} package of \proglang{R} or the more complex \code{zoo} \citep{Zeileis2005} and \code{xts} \citep{Ryan2013} classes implemented in two separate packages. 
...
Argument      &  Description &   Remarks \\ \hline
\code{x}      &      Univariate time series object of type numeric vector, \code{ts}, \code{zoo} or \code{xts}     &             \\ [0.9ex]
\code{y}      &       Univariate time series object of type numeric vector, \code{ts}, \code{zoo} or \code{xts}       &              \\ [0.9ex]
\code{tx}     &      Sampling index of series \code{x} (optional)   & Necessary if \code{x} is a numeric vector and the sampling is not constant.  \\  [0.9ex]
\code{ty}     &      Sampling index of series \code{y} (optional)     & Necessary if \code{y} is a numeric vector and the sampling is not constant.  \\  [0.9ex]
...
\end{tabular}
\caption{Summary of the function \code{tsDistances}.}
\label{tsDistances}
\end{center}
\end{table}
...
The \code{zoo.series1} and \code{zoo.series2} time series included in the package are replicas of the \code{example.series1} and \code{example.series2} objects introduced previously but saved in a \code{zoo} format with a specific time index. 

\begin{example}
A basic call to the \code{tsDistance} function for two series like these is done in the following manner:
...
\end{tabular}
\caption{Summary of the function \code{tsDatabaseDistances}.}
...
\begin{example}
The \code{dist} function calculates the pairwise distance between all the rows in a matrix, so the calculation of the distance matrix can be done easily for the \code{example.database} object:
...
This can also be calculated by using the \code{tsDatabaseDistances} function:
...
The \code{zoo.database} object is not a matrix, so the distance matrix calculation can not be done by using the \code{dist} function directly. In this case, the calculation must be done in the following manner:
...
\section[summary]{Summary and comparison of time series distance packages in \proglang{R}}
...
\begin{table}[h]
\begin{center}
\begin{tabular}{L{7.5cm}  C{1.5cm}  C{1.5cm} C{1.5cm}}  
  &  \pkg{dtw} & \pkg{TSclust}  & \pkg{TSdist} \\ \hline 
Shape-based distances    &  &  &  \\ \hline  
\hspace{0.3cm}$L_p$ distances   &  &  &  \checkmark \\ 
\hspace{0.3cm}Short Time Series Distance (STS) &  &  & \checkmark \\ 
\hspace{0.3cm}Complexity Invariant Time Series Distance  &  & \checkmark &   \\
\hspace{0.3cm}DISSIM  &  &  & \checkmark \\ 
\hspace{0.3cm}Approximated DISSIM &  &  & \checkmark \\ 
\hspace{0.3cm}Dynamic Time Warping (DTW)   & \checkmark &  &  \checkmark \\ 
\hspace{0.3cm}Keogh\_LB (DTW)   &  &  &  \checkmark \\ \hline 
Edit based distances   &  &  &  \\ \hline  
\hspace{0.3cm}Edit Distance for Real Sequences (EDR) &  &  & \checkmark \\ 
\hspace{0.3cm}Edit Distance with Real Penalty (ERP) &  &  & \checkmark \\   
\hspace{0.3cm}Longest Common Subsequence (LCSS) &  &  & \checkmark \\ 
\hline
Feaure-based distances   &  &  &  \\ \hline  
\hspace{0.3cm}Autocorrelation based &  & \checkmark &  \\  
\hspace{0.3cm}Pearson correlation based &  & \checkmark &   \checkmark \\   
\hspace{0.3cm}Cross-correlation based &  &  & \checkmark  \\ 
\hspace{0.3cm}Discrete Fourier Decomposition based &  &  &   \checkmark \\  
\hspace{0.3cm}Wavelet Decomposition based &  & \checkmark &   \\   
\hspace{0.3cm}Periodogram based &  & \checkmark &   \\ 
\hspace{0.3cm}SAX based &  & \checkmark &   \\ 
\hspace{0.3cm}Spectral Density based &  & \checkmark &   \\  
\hspace{0.3cm}TQuest &  &  & \checkmark  \\ \hline  
Structure-based distances    &  &  &  \\ \hline  
\hspace{0.3cm}ARIMA model based &  & \checkmark &  \\ 
\hspace{0.3cm}Compression based &  & \checkmark &  \\   
\hspace{0.3cm} Non Parametric Forecast based &  & \checkmark &  \\  
\end{tabular}
\caption{Summary of distance measures for time series implemented in \proglang{R}.}
\label{summary}
\end{center}
\end{table}
